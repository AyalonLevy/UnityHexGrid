...
public class HexRenderer : MonoBehaviour
{
    ...
    private void DrawFaces()
    {
        ...
        {
            m_faces.Add(CreateFace(innerSize, outerSize, height / 2.0f, height / 2.0f, point));
        }

        // Bottom faces
        for (int point = 0; point < 6; point++)
        {
            m_faces.Add(CreateFace(innerSize, outerSize, -height / 2.0f, -height / 2.0f, point, true));
        }

        // Outer faces
        for (int point = 0; point < 6; point++)
        {
            m_faces.Add(CreateFace(outerSize, outerSize, height / 2.0f, -height / 2.0f, point, true));
        }

        // Inner faces
        for (int point = 0; point < 6; point++)
        {
            m_faces.Add(CreateFace(innerSize, innerSize, height / 2.0f, -height / 2.0f, point, true));
        }
    }
